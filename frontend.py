...
if st.session_state.selected_story is None:
    ...
    story_name = st.sidebar.text_input("Story Name")
    ...
    if st.sidebar.button("Create Story"):
        if not story_name:
            ...
            res = requests.post(f"{BASE_URL}/uploading_doc/{story_name}",files=files)
            if res.status_code == 200:
                st.sidebar.success("Story uploaded successfully!")
                st.rerun()
            else:
    ...
    if st.session_state.messages == []:
        ...
        if res.status_code == 200:
            ...
            for item in data["messages"]:
                if item["role"] == "user":
                    st.session_state.messages.append(("user", item["message"]))
                else:
                    st.session_state.messages.append(("bot", item["message"]))
    ...
    for role, msg in st.session_state.messages:
        if role == "user":
            ...
            with st.chat_message("assistant"):
                st.write(msg)
    user_input = st.chat_input("Continue your story...")
    if user_input:
